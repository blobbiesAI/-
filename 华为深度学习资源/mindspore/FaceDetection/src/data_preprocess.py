...
class SingleScaleTrans:
    '''SingleScaleTrans'''
    def __init__(self, resize, max_anno_count=50):
    ...
    def __call__(self, imgs, ann, image_names, image_size, batch_info):
        ...
        # 一幅图片中的人脸数目不一样，多余200的只保留前200个，少于则用0补齐，保证所有标签矩阵一致性。
...
def build_targets_brambox(img, anno, reduction, img_shape_para, anchors_mask, anchors):
    ...
    if is_empty_or_negative:#图片没有人脸
...
def create_dataset(args):
    ...
    ds = de.MindDataset(args.mindrecord_path, columns_list=["image", "annotation"], num_shards=args.world_size,
                        shard_id=args.local_rank)

    print(ds.get_col_names())
    print("-------------------------")
